...
public class RestPostController {
    ...
    public ResponseEntity<Page<PostDto>> listAllPosts(@RequestParam(defaultValue = "") String demandTypeSearch,
                                                      @RequestParam(defaultValue = "") String lendTypeSearch,
                                                      @RequestParam(defaultValue = "") Double minPriceSearch,
                                                      @RequestParam(defaultValue = "") Double maxPriceSearch,
                                                      @RequestParam(defaultValue = "") String positionSearch, @PageableDefault(page = 0, size = 3) Pageable pageable) {
        Page<PostDto> listPostDtos;
        if (demandTypeSearch != null || lendTypeSearch != null || minPriceSearch != null || maxPriceSearch != null || positionSearch != null) {
            listPostDtos = iPostService.searchAllPost(demandTypeSearch, lendTypeSearch,minPriceSearch,maxPriceSearch,positionSearch , pageable);
        } else {
